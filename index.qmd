...
EAD = (0.87 × 0) + (0.10 × 10,000) + (0.02 × 50,000) + (0.01 × 90,000)  
= 2,900 per year
...
Ans: It hides the variability and high/tail risk. EAD is an average over many years. It doesn't mean every year you will have 2,900 dollar of loss. Maybe , one year you have no flood and zero loss, but in a year, you may have event like Harvey and high loss. So EAD actually average outs this. So, I would try to explain this to homeowner.
Also, 10% probability says, 10 out of 100 year, 2% say 2 out of 100 year etc.
...
```{julia}
#| label: fig-gev-pdf1
#| fig-cap: "GEV distribution for annual maximum flood stage1."
let
...
```{julia}
#| label: fig-gev-explore2
#| fig-cap: "Experiment with different GEV parameters to see how they affect the distribution2."
let
    x_range = range(0, 20; length=200)
    fig = Figure(size=(700, 300))

    # Try changing these values!
    μ_test = 3.0   # Try: 3.0, 5.0, 7.0
    σ_test = 1.0   # Try: 1.0, 2.0, 4.0
...
```{julia}
#| label: fig-gev-explore3
#| fig-cap: "Experiment with different GEV parameters to see how they affect the distribution3."
let
    x_range = range(0, 20; length=200)
    fig = Figure(size=(700, 300))
    ...
    ξ_test = 0.0   # Try: -0.1, 0.0, 0.1, 0.3
...
```{julia}
#| label: fig-gev-explore4
#| fig-cap: "Experiment with different GEV parameters to see how they affect the distribution4."
let
    x_range = range(0, 20; length=200)
    fig = Figure(size=(700, 300))

    # Try changing these values!
    μ_test = 7.0   # Try: 3.0, 5.0, 7.0
    σ_test = 4.0   # Try: 1.0, 2.0, 4.0
    ξ_test = 0.1   # Try: -0.1, 0.0, 0.1, 0.3
...
```{julia}
#| label: fig-gev-explore5
#| fig-cap: "Experiment with different GEV parameters to see how they affect the distribution5."
let
    x_range = range(0, 20; length=200)
    fig = Figure(size=(700, 300))
    ...
    μ_test = 7.0   # Try: 3.0, 5.0, 7.0
    σ_test = 4.0   # Try: 1.0, 2.0, 4.0
    ξ_test = 0.9   # Try: -0.1, 0.0, 0.1, 0.3
...
Ans: When I increased the location parameter from 5 to 7, the mean value across x axis shifts to right.
When I increase the scale parameter from 2 to 4, the distribution spreads out along x axis.
When I increase shape parammeter from 0.1 to 0.3, the tail becomes more fat.
...
# <1> # surge_dist is the already fitted dist, we will randomly take 10,000 samples from here
```
...
Say, you have 10,000 samples. Now divide it like 0-1 ft, 1-2 ft, 2-3 ft etc and count how many values sit on these range, say there are 2000 values from 0-1 ft. so 2000/10000=0.20 will be probability. Then divide it by bin width so 0.20/1 =0.20 will be the density.
so, if you do it for all range and find the density and then do all (Densityxwidth), the sum will be 1.
The area under the histogram equals 1.

The red line is the true pdf, from the real pdf formula say you have many x values(stage) and you have the location,scale and shape aprameters, so then you can determine the red curve values and plot.
...
1. `normalization=:pdf`(the step I did, divided by total samples, and then deivide by bin width) scales the histogram so the total area equals 1. Normalization means converting counts into probability density by dividing by total samples and bin width so the total area equals 1.
...
2. Vertical line showing where the house floor is, so here 2 ft is the elevation, so in x axis, if the flood stage increase, the damage will increase in a S curve style, and after 8ft in x axis, the total house got damaged ( 1x10^5 in the x axis). See, before 2 ft in x axis, the damage was zero.
...
. So from the flood stage historgram, draw 10,000 samples, then for each sample see the corresponding damage from the s shape curve, so now you have 10,000 damage values, then for x axis (Annual damage) divide by 0-10000 dollar, 10000-20000 dollar etc and count how many are there from the 10,000 damage value, say from 0 -10000  there are 2000 random damage, count/10,000 = 2,000/10,000=0.20, then 0.20/bin width 10,000 = density

Again, we plot as a normalized histogram (probability density).
Notice the spike at \$0 (years with no damage) and the long right tail because there are many samples in 0 damage or in high damage.
...
2. Fraction of samples with positive damage (that means any damage greater than 0)
3. Fraction of samples exceeding \$50,000 (any damage greater than 50k)
...
```
damage ratio = corresponding damage / house original value
...
###ok, so we will draw random flood stage, if the stage is greater than 4ft(say we drew 5 ft, so 5-4=1ft is the flood depth above first floor height), then we will have damage ratio from the depth-damage function curve for 1 ft in x axis(say that ratio is 0.10, so damage for 1 ft is 0.10x250,000),then for another sample calculate like this etc..if we drew 3.2 which is less than first floor height, then no damage and the ratio is 0. After getting all sum up and divided by total samples to get EAD.
```{julia}
...
Ans: Damage only occurs, when flood stage is greater than the first floor height, and small flood events are common, rather than the large flood. So if we elevate houses, that means we are safe from small to moderate flood but not rare events. Also, our damage curve is S shaped. These all contributes to no linearity of the EAD in the community.

Why steep drop of EAD as elevation increase? Ans: Because if we increase house elevation a bit from o, we eliminate many small to moderate flood at once and there is a sharp decrease at EAD.
...
Ans: They are equal because the expected damage of the community = sum of expected damage of each house. Some little difference can occur due to monte carlo sample noise.

Different hazard sample will still make them to be equal.
:::
...
Ans: Damage doesn't increase in a straight line with the flood height. The uncertainty in flood heights can propagate into damage. It can create many years with no damage and certain years with high damage.
...
Ans: Say we have avergae flood stage at 4 ft, and from the depth damage curve we compute the damage as $2,000. But in general depth is not always 3 ft. Some years it can be less causing no loss, some year it is high stage causing more loss. So, on avergae the high loss can contribute more and it can dominate the loss. That's why the expected damage is not equal damage at the expected flood stage.
...
Ans: (a) Depth damage curve is same for all houses, (b) One stage applied to etire community (c) uncertainty in flood stage (d) all homes face same flood depth, but in reality their land use, elevation, waterbody proximity etc will vary.
